...
import { Lightbulb, TrendingUp, Database, Search, AlertCircle, History, Zap } from 'lucide-react';
...
type MlPanelState =
  ...
  | { type: 'missing-params' }
  | { type: 'loading' }
  | { type: 'result'; data: MlPredictResult };
...
export default function BudgetSuggestionPage() {
  ...
  const [mlPanel, setMlPanel] = useState<MlPanelState>({ type: 'idle' });
  ...
  const handleSearch = async () => {
    ...
    const canUseMl = isTattoo && !!size && !!complexity && !!bodyLocation;

    if (canUseMl) {
      setMlPanel({ type: 'loading' });
    } else if (isTattoo) {
      setMlPanel({ type: 'missing-params' });
    } else {
      setMlPanel({ type: 'not-tattoo' });
    }
  ...
  // ─── Conteúdo dinâmico do Painel de IA ──────────────────────────────────
  const renderMlPanel = () => {
              ...
              <div className="w-full max-w-xs">
                ...
                <div className="w-full bg-surface-elevated rounded-full h-2">
                  <div
                    ...
                    style={{
                      width: `${Math.min(100, (data.sessionCount / data.minSessionsRequired) * 100)}%`,
                    }}
                  />
                </div>
              </div>
