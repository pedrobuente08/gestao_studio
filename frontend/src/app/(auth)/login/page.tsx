...
export default function LoginPage() {
  ...
  // Observa o campo email para usar no link de reenvio
  ...
  const isEmailNotVerifiedError = (error: Error | null): boolean => {
    ...
    if (error instanceof AxiosError) {
      ...
      return message.includes('Email não verificado');
    }
    return false;
  };
  ...
  const getErrorMessage = (error: Error | null) => {
    if (!error) return null;
    if (error instanceof AxiosError) {
      return error.response?.data?.message || 'Erro ao fazer login. Tente novamente.';
    }
    return error.message || 'Erro ao fazer login. Tente novamente.';
  };

  const emailNotVerified = isEmailNotVerifiedError(loginError);
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-2 text-center text-sm">
        <Link
          href="/reset-password"
          className="block text-rose-500 hover:text-rose-400 transition-colors"
        >
          Esqueci minha senha
        </Link>
